...
def setup
  ...
  @act = build(:act, team: @team)
  @other_acts = create_list(:act, 3)

  @another_act = create(:act, team: @team)
...
def assert_proper_object_serialization(act_data)
  ...
  assert_equal act_data["team_id"], act.team_id
end

test "index" do
  # Fetch and ensure nothing is seriously broken.
  get "/api/v1/teams/#{@team.id}/acts", params: {access_token: access_token}
...
test "create" do
  ...
  act_data = JSON.parse(build(:act, team: nil).api_attributes.to_json)
  act_data.except!("id", "team_id", "created_at", "updated_at")
  params[:act] = act_data

  post "/api/v1/teams/#{@team.id}/acts", params: params
  assert_response :success
  ...
  # Also ensure we can't do that same action as another user.
  post "/api/v1/teams/#{@team.id}/acts",
